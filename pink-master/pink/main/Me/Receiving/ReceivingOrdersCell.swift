import UIKit
protocol ReceivingOrdersCellDelegate: NSObjectProtocol {
    func d_nextClick(model: OrderItem)
    func d_chatClick(userId: String)
}
class LMRecOrdersCell: LMBaseTableViewCell {
    weak var delegate: ReceivingOrdersCellDelegate?
    var model: OrderItem = OrderItem() {
        didSet {
            orderName.lmtext("\(model.itemName) x \(model.number)")
            orderTime.lmtext("时间 \(model.createTime)")
            orderUserName.lmtext(model.targetUserInfo.nickname)
            orderAvatar.set_Image(url: model.targetUserInfo.avatar)
            switch model.status {
            case -1:
                status.image(UIImage(named: "order_un"))
                status.lmtitle("未接单")
                status.titleColor(lmColorHex("#FF9F40FF"))
                nextbtn.backgroundImage(UIImage(named: "order_receivbtn"))
                nextbtn.isHidden = false
                case 0:
                status.image(UIImage(named: "order_no"))
                status.lmtitle("未开始")
                status.titleColor(lmColorHex("#FFDD00FF"))
                nextbtn.backgroundImage(UIImage(named: "order_starbtn"))
                nextbtn.isHidden = false
                case 1:
                status.image(UIImage(named: "order_palying"))
                status.lmtitle("进行中")
                status.titleColor(lmColorHex("#00DBA9FF"))
                nextbtn.backgroundImage(UIImage(named: "order_overbtn"))
                nextbtn.isHidden = false
                case 3:
                status.image(UIImage(named: "order_over"))
                status.lmtitle("已完成")
                status.titleColor(lmColorHex("#1C1C28"))
                case 4:
                status.image(UIImage(named: "order_cancle"))
                status.lmtitle("已取消")
                status.titleColor(lmColorHex("#1C1C2866"))
                case 2:
                status.image(UIImage(named: "order_cancle"))
                status.lmtitle("已取消")
                status.titleColor(lmColorHex("#1C1C2866"))
    ...
    lazy var backImage: UIView = {
        let imageV = UIView()
        imageV.set_Border(radius: 12, borderWidth: 0.5, borderColor: lmColorHex("#1C1C2829"))
    ...
    lazy var sendMsg: UIButton = {
        ...
        btn.backgroundColor(lmColorHex("#1C1C280A"))
        btn.addTarget(self, action: #selector(a_sendMesgClick), for: .touchUpInside)
        return btn
    }()
    lazy var nextbtn: UIButton = {
        let btn = UIButton(lmfont: lmFontR(12), titleColor: .whitePrimary)
        btn.addTarget(self, action: #selector(a_nextbtnClick), for: .touchUpInside)
        return btn
    }()
    ...
    private func set_Subviews() {
        ...
        let lineV = UIView().backgroundColor(lmColorHex("#1C1C2829"))
    ...
    @objc func a_nextbtnClick() {
        delegate? .d_nextClick(model: model)
    }
    @objc func a_sendMesgClick() {
        self.delegate?.d_chatClick(userId: model.targetUserInfo.userId)
    }
}
