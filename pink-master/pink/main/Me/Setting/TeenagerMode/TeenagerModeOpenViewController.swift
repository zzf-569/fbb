import UIKit
extension TeenagerModeOpenViewController {
}
class TeenagerModeOpenViewController: LMBaseVC {
...
private extension TeenagerModeOpenViewController {
    func setViewSnp() {
    ...
    @objc func btnAction() {
        ...
        TeenagerModeManager.shared.open(password)
        if let appDelegate = UIApplication.shared.delegate as? AppDelegate {
            let vc = TeenagerModeViewController()
            vc.isMain = true
            AppConfig.keyWindow.rootViewController = vc
        }
    }
}
