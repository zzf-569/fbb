import UIKit
extension SystemIncomeCell {
    func setDataSoure(_ model: SystemListModel) {
        self.titleLab.text = model.title
        self.amountlb.text = "¥\(model.amount)"
        self.cashlb.attributed.text = "\(.init(string: "消耗贝壳 ".localized, .font(lmFontF(14))), .foreground(lmColorHex("#2B313DA3")))\(.init(string: model.cash.toString(), .font(lmFontF(14))), .foreground(lmColorHex("#2B313D")))"
        self.accountlb.attributed.text = "\(.init(string: "到账账户 ".localized, .font(lmFontF(14))), .foreground(lmColorHex("#2B313DA3")))\(.init(string: model.account, .font(lmFontF(14))), .foreground(lmColorHex("#2B313D")))"
...
class SystemIncomeCell: LMBaseTableViewCell {
    ...
    private lazy var accountlb: UILabel = {
        let lb = UILabel(lmfont: lmFontF(14), textColor: lmColorHex("#2B313DA3"))
        return lb
    }()
...
private extension SystemIncomeCell {
    func setViewSnp() {
        ...
        cashlb.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(accountlb.snp.top).offset(-8.0)
            make.height.equalTo(22.0)
        }
