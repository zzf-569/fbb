import UIKit
class LMRMPackageListView: UIView {
    private lazy var collectionView: UICollectionView = {
        let collectionView = UICollectionView(target: self, cellTypes: [LMRMBageListCell.self])
        collectionView.showsHorizontalScrollIndicator = false
        collectionView.showsVerticalScrollIndicator = false
        return collectionView
    }()
    var page = 1
    private var dataSource: [UserDressModel] = [] {
        didSet {
            collectionView.reloadData()
        }
    }
    var selectedPackageBack: ((UserDressModel) -> Void)?
    var selectedDress: UserDressModel = UserDressModel()
    ...
    func lmrequestData() {
        ShopNetWork.getPackageList(page: page).lmrequest {[weak self] responseModel in
            self?.selectedDress = UserDressModel()
...
extension LMRMPackageListView: UICollectionViewDataSource & UICollectionViewDelegateFlowLayout & UICollectionViewDelegate {
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        self.dataSource.count
    }
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(cellType:LMRMBageListCell.self, cellForRowAt: indexPath)
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        self.selectedPackageBack?(dataSource[indexPath.row])
        collectionView.reloadData()
    }
}
extension LMRMPackageListView: JXSegmentedListContainerViewListDelegate {
