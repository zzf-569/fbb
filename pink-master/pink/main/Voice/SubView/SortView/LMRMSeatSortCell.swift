import UIKit
class LMRMSeatSortCell: LMBaseTableViewCell {
    ...
    lazy var onSeatbtn: UIButton = {
        let btn = UIButton(image: UIImage(named: "rm_seat_sequence_on_seat"))
        return btn
    }()
    lazy var removebtn: UIButton = {
        let btn = UIButton(image: UIImage(named: "rm_seat_sequence_remove"))
        return btn
    }()
...
private extension LMRMSeatSortCell {
    func setViewSnp() {
        ...
        userusheaderView.snp.makeConstraints { make in
            make.left.equalTo(numlb.snp.right).offset(10.0)
            make.centerY.equalToSuperview()
        ...
        onSeatbtn.snp.makeConstraints { make in
            make.right.equalTo(removebtn.snp.left).offset(-5.0)
            make.centerY.equalToSuperview()
