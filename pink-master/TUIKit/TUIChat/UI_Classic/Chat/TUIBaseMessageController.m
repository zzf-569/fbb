...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.delegate respondsToSelector:@selector(messageController:onShowMessageData:atIndexPath:)]) {
        cell = [self.delegate messageController:self onShowMessageData:data atIndexPath:indexPath];
        if (cell) {
            cell.delegate = self;
            return cell;
        }
...
- (void)addChatCommonActionToCell:(TUIMessageCell *)cell ofMenu:(TUIChatPopMenu *)menu {
    ...
    TUIChatPopMenuAction *reportAction = [self setupReportAction:cell];
...
//举报按钮 weight 1000
- (TUIChatPopMenuAction *)setupReportAction:(TUIMessageCell *)cell {
    @weakify(self);

    TUIChatPopMenuAction *groupPinAction = nil;
    UIImage* img = TUIChatBundleThemeImage(@"chat_icon_group_pin_img", @"icon_pin");
    groupPinAction = [[TUIChatPopMenuAction alloc] initWithTitle:@"举报" image:img weight:1000 callback:^{
        @strongify(self);
        [self onReportSelect:nil];
    }];
    return groupPinAction;
}
...
- (void)onReportSelect:(id)sender {
    [self enableMultiSelectedMode:YES];
    if (self.menuUIMsg.innerMessage.hasRiskContent) {
        if (_delegate && [_delegate respondsToSelector:@selector(messageController:onSelectMessageMenu:withData:)]) {
            [_delegate messageController:self onSelectMessageMenu:2 withData:nil];
        }
        return;
    }
